...
def splits(symbol: str) -> List[dict]:
    try:
        # 1. 获取拆股数据 (返回 Series)
        splits_series = yf.Ticker(symbol).splits
...
def actions(symbol: str) -> List[dict]:
    try:
        # 1. 获取公司行动数据 (返回 DataFrame)
...
def capital_gains(symbol: str) -> List[dict]:
    try:
        # 1. 获取资本利得数据 (返回 Series)
        gains_series = yf.Ticker(symbol).capital_gains
...
def get_shares_full(
    ...
    try:
        ...
        shares_series = yf.Ticker(symbol).get_shares_full(start=start, end=end)
...
def fast_info(symbol: str) -> dict:
    try:
        # 1. 直接获取原生完整的极速数据对象
        raw_fast = yf.Ticker(symbol).fast_info
...
def news(symbol: str, count: int = 10, tab: str = "news") -> list[dict]:
    try:
        # 1. 直接获取原生数据
        # 核心注意：它返回的就是 list[dict]，所以不需要 df.to_dict(orient="records")
